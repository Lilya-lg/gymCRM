...
class TraineeServiceImplTest {
  ...
  @Mock private TrainingMessageProducer messageProducer;
  ...
  void deleteProfileByUsername() {
    ...
    when(trainingRepository.findTrainersByTraineeId(1L)).thenReturn(List.of(trainer));
    when(trainingRepository.findByCriteria(username, null, null, null, "trainerUser"))
  ...
  void getTraineeProfile_withValidUser_returnsProfile() {
    ...
    when(traineeRepository.findByUsername(username)).thenReturn(Optional.of(trainee));
    when(trainingRepository.findTrainersByTraineeId(trainee.getId()))
